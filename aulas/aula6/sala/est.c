...
union RGBA_Pixel {
  ...
  // Exercício 1
    ...
    union RGBA_Pixel *pixels = (union RGBA_Pixel *)pchar2;
    
    (*(pixels +0)).rgba[3]=0;
    ((pixels +0))->rgba[3]=0;
    pixels[0].rgba[3]=0;
    pixels[0].pixel = 0x00FFFFFF;
    
  // soluçao:
  //unsigned char *pchar2 = (unsigned char *)&imagem2[0][0];
    //int i;
    //for (i=3; i<10*10*4; i+=4){
     // pchar2[i]=0;         
    //}

  // Exercício 2
    unsigned char *pchar3 = (unsigned char *)imagem3;
    union RGBA_Pixel *pixel_s = (union RGBA_Pixel *)pchar3;

    (*(pixels +0)).rgba[3]=0;
    ((pixels +0))->rgba[3]=0;
    pixels[0].rgba[3]=0;
    pixels[0].pixel = 0x00000011;

  // insira o seu código abaixo
    //int j=0;   
    //int media=0;
      //for (int j=0; j<10*10*4; j+=4){
        //int media = round((pchar3[j+1]+pchar3[j+2]+pchar3[j+3])/3.0);
        //pchar3[j+1]=media;
        //pchar3[j+2]=media;
       //pchar3[j+3]=media;
